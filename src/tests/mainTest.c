...
int main(int argc, char *argv[]){
    ...
    */
   if(argc < 4){
    ERR_PRINT("NEED 3d argument to select test... exiting here");
    close_editor();
    exit(-1);
   }
   srand(time(NULL));
   ...
   for(int i = 0; i < 20; i++){
    ...
    // >>> INSERTS profiler (to be run multiple times on different files with different sizes):
    int selector = atoi(argv[3]);
    ERR_PRINT("Running with selector:%d",selector);
    if (selector == 0){
        ...
        for (int amt = 4; amt < 1005; amt++){
            ...
            if (amt%2 == 0){
                ...
                profilerStart();
                insert(activeSequence, 5, randomString);
                profilerStop("Opti_Insert");
            }
            profilerStart();
            testingRefreshAdaptation();
